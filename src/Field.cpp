	...
	for (ptrdiff_t i = 0; i < local_dim0_size; ++i) {
		for (ptrdiff_t j = 0; j < N; ++j) {
			for (ptrdiff_t k = 0; k < INDEX_RIGHT; ++k) {
				idx = (i * N + j) * (N / 2 + 1) + k;
				coef_0 = indeces[local_dim0_start + i];
				coef_1 = indeces[j];
				coef_2 = k;
	...
	for (ptrdiff_t i = 0; i < local_dim0_size; ++i) {
		for (ptrdiff_t j = 0; j < N; ++j) {
			for (ptrdiff_t k = 0; k < INDEX_RIGHT; ++k) {
				...
				coef_0_r = k;
				coef_1_l = k;
				coef_1_r = indeces[local_dim0_start + i];
				coef_2_l = indeces[local_dim0_start + i];
				coef_2_r = indeces[j];
...
double Field::energy_fourie() const {
	...
	for (ptrdiff_t i = 0; i < local_dim0_size; ++i) {
		for (ptrdiff_t j = 0; j < N; ++j) {
			...
			for (ptrdiff_t k = 1; k < INDEX_RIGHT; ++k) { //  [1, N/2 + 1)
				++idx;
	...
	for (ptrdiff_t q = 0; q < 3; ++q) {
		for (ptrdiff_t i = 0; i < local_dim0_size; ++i) {
			for (ptrdiff_t j = 0; j < N; ++j) {
				for (ptrdiff_t k = 0; k < N; ++k) {
					idx = (i * N + j) * (2 * (N / 2 + 1)) + k;
					cur_x = RANGE_RIGHT * (local_dim0_start + i) / N;
